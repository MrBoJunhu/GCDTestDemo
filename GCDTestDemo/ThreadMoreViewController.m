...
@interface ThreadMoreViewController ()
...
@implementation ThreadMoreViewController
...
- (void)addButton {
    ...
    [cancelLoadBtn addTarget:self action:@selector(cancelLoadImages:) forControlEvents:UIControlEventTouchUpInside];
    
    [cancelLoadBtn setTitle:@"停止加载" forState:UIControlStateNormal];
...
- (void)loadImageWithMultiThread:(UIButton *)sender {
    ...
    NSUInteger threadCount = self.imagesArray.count;
    
    //创建多个线程用于填充图片
    方法一 :
        _threads=[NSMutableArray arrayWithCapacity:threadCount];
        ...
        for (int i = 0; i< threadCount; i++) {
            ...
            thread.name = [NSString stringWithFormat:@"线程   %i",i];//设置线程名称
...
#pragma mark - 为了简化多线程开发过程，苹果官方对NSObject进行分类扩展(本质还是创建NSThread)，对于简单的多线程操作可以直接使用这些扩展方法。
//    //方法二 (不可以取消线程):
//    for (int i = 0; i < threadCount; ++i) {
//        
...
- (void)cancelLoadImages:(UIButton *)sender {
    
    sender.userInteractionEnabled = NO;
    
    for (NSUInteger i = 0 ; i  < self.threads.count; i ++) {
        
        NSThread *currentThread = self.threads[i];
